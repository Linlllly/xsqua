		...
		>
			<div class="look-title">
				<img
					...
					alt=""
					@click="toOtherUser(i.userInfo, 1)"
				/>
				<div class="info-des">
...
export default {
	...
	mounted() {
		this.fillerIdList = uni.getStorageSync(this.filler)
			? uni.getStorageSync(this.filler)
			: []
		this.getTakeLook()
	},
	onShow() {
	...
	methods: {
		...
		async getTakeLook(n) {
			...
			// ** 打开节流阀
			this.isloading = true
			let res = await takeLook({
				limit: this.limit,
				fillerIdList: this.fillerIdList,
				meeting: this.type
			})
			console.log('请求瞧一瞧列表')
			console.log(res)
