<template>
	<div class="pages">
		...
		<div class="get-gift" @click="showGetMaterial = true">继续填写兑换地址</div>
		...
		<div class="record" v-for="i in recordList" :key="i">中奖记录-----呜呜呜呜呜呜呜呜呜呜呜呜呜呜呜呜呜呜呜呜呜1111111111111111111111111111</div>
		<div class="area-all">
		...
		<div class="who-are-you">
			<image src="https://www.zairongyifang.com:8080/filePath/app/20241/compressed_c99ab46582.jpg" mode="widthFix" style="width: 100%" @click="toOtherUser()"></image>
		</div>
		...
		<!-- 确认兑换吗 -->
		<u-modal title="兑换" :show="showClaim" @confirm="confirmClaim" confirmColor="#e89406" showCancelButton @cancel="showClaim = false">
			<view class="slot-content">确认兑换xxx？</view>
		</u-modal>
		<!-- 填写收货信息弹窗 -->
		<u-modal title="填写收货信息" :show="showGetMaterial" @confirm="confirmGetMaterial" showCancelButton @cancel="showGetMaterial = false" confirmColor="#e89406">
...
import { prize, draw, prizeRecord, receive, exchangeMaoTai } from '@/api/game.js';
	...
	methods: {
		...
		confirmGetMaterial() {
			this.$refs.materialForm
				.validate()
				.then((res) => {
					const request = this.prizeInfo.id ? receive({ id: this.prizeInfo.id, ...this.materialForm }) : exchangeMaoTai(this.materialForm);
					request.then((res) => {
						if (res.code !== 0) {
							uni.$u.toast(res.msg);
							return;
