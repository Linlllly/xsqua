...
export default {
	...
	onLoad() {
		this.getBanner();
		this.fillerIdList = uni.getStorageSync('filler3') ? uni.getStorageSync('filler3') : [];
		this.getTakeLook();
	},
	onShow() {
		this.refresh = true;
	...
	methods: {
		...
		async getTakeLook(n) {
			...
			// ** 打开节流阀
			this.isloading = true;
			let res = await takeLook({ limit: this.limit, fillerIdList: this.fillerIdList, meeting: 3 });
			console.log('请求瞧一瞧列表');
			console.log(res);
			if (res.code !== 0) {
				uni.showToast({
			...
			uni.setStorageSync('filler3', this.fillerIdList);
