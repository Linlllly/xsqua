					...
					<block v-if="row.type !== 'err_msg'">
						...
						<view v-if="row.fromUid == uid" class="my" @longpress="recordOkBtn(row)">
							...
							<view class="left">
								...
								<!-- 文字消息 -->
								<view v-if="row.type === 'chat'" class="bubble" :id="'bubble' + row.id" style="position: relative" @longpress="setControls(row, $event)">
									<text>{{ row.text }}</text>
								</view>

								<!-- 图片消息 -->
								<view v-if="row.type === 'chat_image'" class="bubble img" @tap="showPic([row.text])" :id="'bubble' + row.id" @longpress="setControls(row, $event)">
		...
		<u-modal :show="showRemove" title="删除" @confirm="confirmRemove" showCancelButton @cancel="showRemove = false" confirmColor="#e89406">
			<view class="slot-content">
				<u-radio-group v-model="cleanType" placement="column" activeColor="#e89406" size="20">
					...
					<u-radio :name="1" shape="circle" :label="cleanAll === true ? '一键删除双方手机上所有聊天记录' : '一键删除双方手机上这条聊天记录'"></u-radio>
				</u-radio-group>
			</view>
		</u-modal>
		<!-- 改动 -->
