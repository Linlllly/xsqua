	...
	<view class="pages">
		...
		>
			...
			>
				...
				<view
					class="row"
					v-for="(row, index) in chatWithList"
					:key="index"
					:id="'msg' + row.id"
				>
					<!-- 用户消息 -->
					<block v-if="row.type !== 'err_msg'">
						<!-- 111111自己发出的消息 -->
						<view
							v-if="row.fromUid == uid"
							class="my"
							@longpress="recordOkBtn(row)"
						>
		...
		>
			...
			<text v-if="popupRow.type === 'chat'" style="margin: 0 10rpx"
				>|</text
			>
			<text v-if="popupRow.type === 'chat'" @click="copyMessage"
				>复制</text
			>
		</div>
	</view>
				...
				if (this.chatWithList.length === 0) {
					...
					queryss.exec((res) => {
						res[0].forEach((v, i) => {
							...
							if (
								Math.ceil(allMessHeight) >= this.contentHeight
							) {
								//结束循环
								return
							}
						...
						if (Math.ceil(allMessHeight) >= this.contentHeight) {
							...
							this.contentHeight - this.messBotton >=
							Math.ceil(allMessHeight)
						) {
							//没超过可视区-键盘和输入框高 不上弹
							this.inputBotton = 0
						} else {
				...
				success: () => {
					...
					this.$nextTick(() => {
						...
						if (this.changeSendId) {
							this.$set(
								this.chatWithList[this.chatWithList.length - 1],
								'id',
								'0_' + Date.now().toString()
							)
		...
		async confirmRemove() {
			...
			deleteHistory(obj).then((res) => {
				...
				if (this.cleanAll) {
					...
					const index = this.chatWithList.findIndex(
						(item) => item.id === this.popupRow.id
					)
