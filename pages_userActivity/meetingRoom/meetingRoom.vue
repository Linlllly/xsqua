<template>
	<div class="pages">
		...
		<div class="dynamic">
			<div class="title">
				空间会议室
				<u-icon name="plus" color="#fff" size="22" @click="toIssue"></u-icon>
				<div :style="{ fontWeight: 500, marginLeft: '16rpx', fontSize: '40rpx', color: '#f9ae3d' }">发布</div>
			</div>
		</div>
		<!-- 列表 -->
...
<script>
import { spaceMeetingPage } from '@/api/meetingRoom/meetingRoom.js';
...
export default {
	...
	methods: {
		//获取列表
		async getSpaceMeeting() {
			// ** 打开节流阀
			this.isloading = true;
			let res = await spaceMeetingPage({ page: this.page, limit: this.limit });
			console.log('请求会议室列表');
			console.log(res);
			if (res.code !== 0) {
				uni.showToast({
			...
			//data请求评论
			this.linsMeeting = res.result.data;
			...
			for (var i = 0; i < this.meetingList.length; i++) {
				...
				if (this.meetingList[i].img && this.meetingList[i].media.length === 1) {
					// console.log(this.meetingList[i].media[0]);
				}
		...
		toIssue() {
			...
			uni.navigateTo({
				url: '../artcleIssue/artcleIssue?meeting=' + 1
			});
		},
		//单图预览
		previewImg(i, index) {
...
.dy-content {
	...
	box-sizing: border-box;
	padding: 10rpx 16rpx;
	border-bottom: 2rpx solid #616161;
	.look-title {
		display: flex;
